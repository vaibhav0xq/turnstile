...
Status: This is a planning record. Direction A was approved on 15 Sep 2026 and built. The current product is described in the root README.
...
Phase A (`16eb253`, `d086bfc`) shipped the mechanics: a scroll-driven flight, the `/` ↔ `/city` split and a
...
1. **One scene, one altitude, one camera family.** The landing reuses the picker's world _and_ its
   viewpoint: every landing key sits 45 to 125 m up looking down at the same block field the picker shows
...
2. **Two things changed at once and neither was a story beat.** The hero looks toward the horizon (where
   the dusk band lives) with thinner haze; frames I to III look down at dark ground with thicker haze. The
   effect is "weather", not narrative.
3. **The masses carry no detail where the eye lands.** Windows are screen-space points (2 to 9 px dots), so
...
The spike below shows that (3) is cheap to fix and (1) to (2) are design decisions, not engineering ones.
...
It also showed what does _not_ work yet: a **street-level** camera (2 to 4 m up) is inside the ground haze,
...
(the picker). Same world, opposite altitude language: the landing lives at 2 to 40 m and looks _across_ the
...
| 2 | **Massing variety**: 2 to 3-tier setbacks on tall buildings, crowns/spires/antennae, rooftop mech boxes, a few slab-vs-tower proportions | high | 1 d | all | must |
...
| 6 | **Venue façades** at the two beacons (theatre with marquee, club as a low pavilion with neon) + 4 to 6 **signage** panels (canvas-drawn, abstract or event names) | medium-high | 1 d | all | should |
...
| --- | --- |
| Hero + beats I to IV, _Tonight in the city_ (bill as preview), footer | Map at `CITY_POSE`, beacon labels + leader lines, the bill as the working list |
...
- **A · "The Door, then the City"** (recommended). Everything above. City upgrades 1 to 5 + 9 (5.5 d), 6 to 7
...
- **B · "Skyline, then the City"** (−2 days). The hero is the 26 m skyline frame from the spike with a
  rooftop or water foreground instead of gates; beats I to III at 20 to 40 m; no plazas/gates. Loses the
...
| --- | --- | --- | --- |
| 1 | 15 to 21 Sep | City v2 core: façades, massing, streets, plazas + gate prototype, one-night lighting, tiers | Republish staging; phone + desktop check; go/no-go on the gate |
| 2 | 22 to 28 Sep | Landing v2: hero, beats, device frames, map hand-over, `/city` console, mobile; venue façades + signage + spire; waterfront if ahead | Republish; full walkthrough on phone; submission form opened with placeholders |
| 3 | 29 Sep to 5 Oct | Product: §4.4 theatre reframe, §4.5 seat view in followspot, §4.6 finale, §4.8 mobile tier; fixes from device testing; copy pass | Republish; judge run green on staging |
| 4 | 6 to 13 Oct | Demo video (site frozen first), README/submission, buffer | Submitted ≥ 24 h early |
...
- **Scope in the city.** Items 6 to 8 are the ones to cut; 1 to 5 are not negotiable for either direction.
