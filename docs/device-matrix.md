...
**Status on 18 Sep 2026.** Verified on real devices: Android Chrome with Google Password Manager (full path
on the phone, stateless recovery after a wipe) and a Windows laptop through the hybrid QR sheet with that
phone as the authenticator. The other rows are unverified and optional. They are not a blocker for the
submission: a provider without PRF gets a clear `PRF_UNAVAILABLE` message in the product instead of a
silent failure and the copy names the verified combinations. iOS and macOS rows will be filled in as
