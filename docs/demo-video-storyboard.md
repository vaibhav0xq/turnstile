...
Totals on the dry run: **38.2 s on the bar · 6 taps · 2 passkey prompts · 3 transactions**. The recorded take
reads 47.1 s because the capture holds real time around the prompts and the door. Locally against anvil the
same run reads ~36 s; the difference is testnet confirmation time, which is the honest number to show.

The bar keeps counting while a human reads the screen, so a leisurely take ends nearer 60 to 80 s. Two ways to
...
- **Natural pace (recommended).** Let autoplay drive (`?tour=auto`), only touch the two passkey prompts, and
  accept a bar under a minute. Stretch the cut to two minutes with holds and the B-roll below. The final number
  on the summary card is whatever the take produced.
...
- A free front-row seat: the tier chip picks the front-most open seat nearest the centre and every run
  consumes one, so the take lands on Row A · 14 / 16 / 13…. This is fine, but do not expect seat 15 again.
...
| 7 | 1:16-1:32 | 0:47-0:56 | Finale: cut back to the room, camera drops behind the seat, the **followspot** finds it and the floor ring rolls out. Nothing else is in the beam. Summary card: time, taps, two prompts, three hashes. | "Seat lit from chain state. One identity, two prompts, three transactions, well under a minute." |
...
| View from here, 8 s | after the finale press `Explore the room` (not the seat, since clicking a selected seat deselects it), then *View from here* on the returned card; hold on the wall | 8 |
...
- Subtitle the two prompts ("passkey · buy", "passkey · bind door key"), so viewers on mute should still count
  them.
...
- If a take fails (RPC hiccup, mis-tap), `restart` on the bar starts a cold run with a new passkey and the same seat
